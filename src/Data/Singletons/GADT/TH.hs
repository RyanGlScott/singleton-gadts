...
genSingletons names = do
  ...
  newDecs <- singKindInstances names
  pure $ oldDecs ++ newDecs
...
singletons = singletons' TH.singletons
...
singletonsOnly = singletons' TH.singletonsOnly

singletons' :: OptionsMonad q => (OptionsM q [Dec] -> OptionsM q [Dec]) -> q [Dec] -> q [Dec]
singletons' genSings qdecs = do
  decs <- qdecs
  decs' <- withOptions noOldSingKindOptions $ genSings (pure decs)
  ddecs <- singDecs decs
  pure $ decs' ++ decsToTH ddecs
